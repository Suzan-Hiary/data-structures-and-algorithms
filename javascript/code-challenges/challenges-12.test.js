...
const replaceZeros = (string) => {
  ...
  let newArray = string.replace(/0/g,'zero') 
  return newArray ;
}
...
const validateWord = (word) => {
  let regularExp = /\b[A-Za-z]{5,10}\b/
  return regularExp.test(word)
};
...
const validateEmail = (email) => {
  let regularExp = /^\w+\.?\w+@\w+.(com|net|org)$/
...
const validatePhoneNumber = (phoneNumber) => {
  let regularExp = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/
  return regularExp.test(phoneNumber)
};
