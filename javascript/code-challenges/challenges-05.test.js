...
const toLastNames = people => {
  ...
  let array = [];
  people.map(item => {
    array.push([item.firstName, item.lastName].join(" "))
...
const addValues = (arr) => {
  ...
  let finalvalue = arr.reduce(getSum, 0);
  return finalvalue;
...
const addPurchases = (arr) => {
  ...
  let totalpurchased = arr.reduce((prev,cur) => {
  return cur.purchasePrice + prev ;}
  , 0);
...
const countNumberOfElements = (arr) => arr.reduce((acu , cu) => cu ? acu + 1 : ac + 0, 0);
...
const returnNames = (arr) => {
  // Solution code here...
 let names=[];
  starWarsData.reduce( (acc, currentValue)=> {
...
const reversedString = (str) => {
  // Solution code here...
  str.split('').reduce(((reverse, character) => character + reverse), '');
};
